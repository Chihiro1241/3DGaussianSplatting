#!/usr/bin/env bash
# 4DGS_warmstart.sh — Neu3D の動的シーンを warm-start で回す。
# 前フレームの学習結果を次フレームの初期値として引き継ぐ。引き継ぐのは
# ガウシアンパラメータだけで、Adam のモーメント・位置 LR スケジュール・
# ADC 統計は毎フレーム作り直される (extensions/4dgs/trainer_4d.py の
# build_frame_training_state を参照)。
#
#   runs/4DGS_warmstart.sh                         # coffee_martini / 300 フレーム
#   runs/4DGS_warmstart.sh cook_spinach 100
#   setsid nohup runs/4DGS_warmstart.sh > logs/4dgs_warmstart.log 2>&1 &
#
...
echo "=============================================================="
echo " 4DGS warm-start  開始 $(date '+%Y-%m-%d %H:%M:%S')"
...
if want train; then
    ...
    if [ "$START" -gt "$END_FRAME" ]; then
        ...
        if [ "$START" -gt 1 ]; then
            ...
            echo "引き継ぎ元: $carry"
            train_command+=(--carry_over_checkpoint "$carry")
        fi
        run_step "${train_command[@]}" \
